<!DOCTYPE html>
<!-- BUILD_MARKER: v2026-07-12-H (מוסיף: 'תצוגה מינימלית' לציר-הזמן - V1 ראשוני, תכונה חדשה נפרדת לגמרי, לא נוגעת בתצוגה הרגילה) -->
...
<div class="main">
  ...
  <div id="tab-timeline" class="panel">
    ...
    <div id="tl-minimal-container" style="display:none;overflow-x:auto;overflow-y:visible;border:1px solid var(--border);border-radius:8px;background:var(--surface)"></div>
  </div>
...
function findNextCandlesAndHavdalah(fromDate){
  ...
  for(let i=0;i<10 && (!nextCandles||!nextHavdalah);i++){
    ...
    if(!nextCandles && entry['שקיעה'] && isAssurMelachaEntryClient(tEntry)){
      if(i>0 || !timeAlreadyPassed(entry['שקיעה'], fromDate)){
        nextCandles={date:d, time:entry['שקיעה'], label:labelForCalEntry(tEntry)};
      }
    }
...
function getZmanMinutesForDateSM(zmanVal, dayDate){
  ...
  if(zmanVal==='candles' || zmanVal==='havdalah'){
    ...
    if(zmanVal==='candles'){
      ...
      return timeStrToMinutesLocal(entry['שקיעה']);
    } else {
...
function getTimeBlocksForDate(date, daySegments){
  ...
  if(daySegments && daySegments.length){
    const dayStart=date.getTime(), dayEnd=dayStart+1440*60000;
    ...
    p.relay.forEach(rId=>{
      getRelayEventPairs(p,rId).forEach(seg=>{
        if(seg.endMin!==null && seg.endMin>1440){
          carriedOverEnds.push({p, rId, endMin: seg.endMin-1440, segAction: seg.action});
        }
      });
    });
  });
...
function renderTimelineMinimal(){
  ...
  dates.forEach(dayDate => {
    ...
    programs.filter(p => p.active && !p.parentProgId && progAppliesOnDate(p, dayDate)).forEach(p => {
      ...
      p.relay.forEach(rId => {
        ...
        getRelayEventPairs(p, rId).forEach(seg => {
          if (seg.endMin === null || seg.endMin === undefined) return; // בלוק-פתוח-ללא-סוף — לא נתמך בגרסה זו עדיין
          relaySegs[rId].push({
            startMs: dayStart + seg.fireMin*60000, endMs: dayStart + seg.endMin*60000,
            isOn: seg.action === 'ON',
            kind: seg.action === 'ON' ? 'on' : (seg.isEndEvent ? 'duration-end' : 'off-action'),
            progId: p.id, label: p.name
          });
        });
      });
      ...
      if (child && child.active && child.relay && child.relay[0] && relaySegs[child.relay[0]]) {
        getRelayEventPairs(child, child.relay[0]).forEach(seg => {
          if (seg.endMin === null || seg.endMin === undefined) return;
          relaySegs[child.relay[0]].push({
            startMs: dayStart + seg.fireMin*60000, endMs: dayStart + seg.endMin*60000,
            isOn: seg.action === 'ON',
            kind: seg.action === 'ON' ? 'on' : 'duration-end',
            progId: child.id, label: child.name
          });
        });
      }
    });
  ...
  const activeRelays = [];
  relays.forEach(r => {
    const segs = (relaySegs[r.id]||[]).filter(s => s.endMs > rangeStartMs && s.startMs < rangeEndMs);
    ...
    segs.forEach(s => {
      ...
      if (st > cursor) filled.push({ startMs: cursor, endMs: st, isOn: false, kind: 'gap', progId: null, label: '' });
      if (en > st) filled.push({ ...s, startMs: st, endMs: en });
      cursor = Math.max(cursor, en);
    });
    if (cursor < rangeEndMs) filled.push({ startMs: cursor, endMs: rangeEndMs, isOn: false, kind: 'gap', progId: null, label: '' });
  ...
  activeRelays.forEach(({relay,segs}) => {
    html += `<div style="min-width:150px;flex-shrink:0;border-left:1px solid var(--border)">
      <div style="position:sticky;top:0;background:var(--surface2);padding:6px;text-align:center;font-weight:700;font-size:12px;border-bottom:1px solid var(--border);z-index:2">${relay.name}</div>
      <div>`;
    segs.forEach(s => {
      const durMin = (s.endMs - s.startMs)/60000;
      const h = Math.round(clampPx(durMin));
      const startStr = fmtEpochHM(s.startMs), endStr = fmtEpochHM(s.endMs);
      const clickable = s.kind === 'on' || s.kind === 'off-action' || s.kind === 'duration-end';
      const bg = s.kind === 'gap' ? '#2a3142' : (s.isOn ? 'rgba(34,197,94,0.75)' : 'rgba(239,68,68,0.65)');
      ...
      html += `<div style="height:${h}px;background:${bg};${cursorStyle};display:flex;align-items:center;justify-content:center;font-size:10px;color:#fff;border-bottom:1px solid rgba(0,0,0,0.35);overflow:hidden"${onclickAttr} title="${title}">${h>=16?startStr+'–'+endStr:''}</div>`;
    });
    html += '</div></div>';
  });
  html += '</div>';
